...
public class ArgumentExprConnector extends ExprConnector {
    ...
    public ArgumentExprConnector(int x, int y, CodeWidget owner, int argIndex) {
        super(x, y);
        this.owner = owner;
        this.argIndex = argIndex;
    }

    public static void parse(Font font, int aX, int aY, String key, List<ExprCodeWidget> args, ExprCodeWidget owner, Consumer<Connector> collector) {
    ...
    public void insert(@Nullable CodeWidget widget) {
        ...
        owner.insertById(argIndex, (ExprCodeWidget) widget);
    }

    @Override
    public CodeWidget get() {
        return owner.getByIndex(argIndex);
    }
}
