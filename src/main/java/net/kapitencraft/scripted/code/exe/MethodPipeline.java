...
public class MethodPipeline<T> {
    ...
    private final List<Method<?>.Instance> functions;
    private final boolean isLoop;
    private boolean canceled, broken, continued, stopped;
    private VarMap map;
    private T ret;

    public MethodPipeline(VarType<T> returnType, List<Method<?>.Instance> functions, boolean isLoop) {
    ...
    private static List<Method<?>.Instance> readInstances(JsonObject object, VarAnalyser analyser) {
        ...
        JsonHelper.castToObjects(array)
                .map(object1 -> JsonHelper.readMethodChain(object1, analyser))
                .forEach(list::add);
        return list;
    }
    ...
    public JsonElement toJson() {
        ...
        this.functions.stream().map(Method.Instance::toJson).forEach(array::add);
